export default function StatCards({ stats, view, warehouse = 'US' }) {
  if (warehouse === 'EU') {
    const cards = [
      {
        primary: true,
        ...
        sub: 'EU Warehouse · all aisles'
      },
      {
        value: stats.emptyLocations ?? 0,
        label: 'Empty Locations',
        sub: 'Empty boxes across all levels'
      }
    ]
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        {cards.map((card, i) => (
          ...
          >
            <div className={`text-3xl font-bold tabular-nums ${card.primary ? '' : 'text-slate-900 dark:text-white'}`}>
              {card.value}
            </div>
            <div className={`text-sm mt-1 ${card.primary ? 'text-blue-100' : 'text-slate-500 dark:text-slate-400'}`}>
              {card.label}
            </div>
            <div className={`text-xs mt-1 ${card.primary ? 'text-blue-200/70' : 'text-slate-400 dark:text-slate-500'}`}>
              {card.sub}
            </div>
          </div>
        ))}
